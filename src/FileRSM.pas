...
var
  ...
  Skin, Fskin, t_pri: array of Integer;
...
//nope fucking wrong - this is for new format not old - i1, someoffset2 and someoffset7 are the wrong things to assign.
  SetLength(V, 0); SetLength(V, inumvert);
  SetLength(Skin, 0);
  IF i_24 > 0 then  SetLength(Skin, inumvert);
  for i := 0 to inumvert -1 do begin
  V[i][0] := vert_list[i][0]; V[i][1] := vert_list[i][1]; V[i][2] := vert_list[i][2];
  IF i_24 > 0 then Skin[i] := vert_skin[i];
  end;
  ...
  SetLength(Fskin, 0);
  SetLength(t_pri, 0);
  IF i_22 > 0 then  SetLength(Fskin, inumtri);
  IF i_20 = 255 then  SetLength(t_pri, inumtri);
  IF i_20 < 255 then Gi1 := i_20;
  for i := 0 to inumtri-1 do begin
  F[i][0] := tri_list[i][0]; F[i][1] := tri_list[i][1]; F[i][2] := tri_list[i][2];
  C[i] := tri_color[i];
  IF i_22 > 0 then Fskin[i] := tri_skin[i];
  IF i_20 = 255 then t_pri[i] := tri_pri[i];

 // MessageBox(0, PChar('oooops'), PChar('Error'), MB_OK);
  //  A[i] := 0;
...
//  AssignFile(Fout2, 'debug.txt');
 // Rewrite(Fout2);
  ...
  try
    ...
    inumvert := HeaderStream.ReadUnsigned(2);
    inumtri := HeaderStream.ReadUnsigned(2);
...
//set texture arrays after reading rest of file. then read textures
 		...
 		stream.LoadFile(FileName); stream.SetOffset(i_36);
...
if (i_18 > 0) then begin
   ...
   MessageBox(0, PChar('Unable to load'), PChar(FileName), MB_OK);
end;
...
//     else MessageBox(0, PChar('zoofile = true'), PChar('true'), MB_OK);
 ...
 //   HeaderStream.SetOffset(0);
   ...
   if(HeaderStream.ReadByte = 255) AND
      ...
      // LoadRSMold(FileName);
      LoadRSMnew(FileName);
      If (didload = True) then Model_version := 2;
      end else
      begin
      didload := False;
      LoadRSMold(FileName);
      If (didload = True) then Model_version := 1;
      //assign data
      end;
  except
    IsValid := false;
